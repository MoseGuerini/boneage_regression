...
class DataLoader:
    ...
    # Getter and Setter: preprocessing
    @property
    ...
    def preprocessing(self, value):
        if not isinstance(value, bool):
            raise ValueError(f"Invalid preprocessing: {value}. preprocessing must be a boolean (True or False).")
    ...
    def preprocess_images(self):
        ...
        eng.preprocessing(str(pathlib.Path(__file__).resolve().parent.parent / 'Test_dataset'), str(pathlib.Path(__file__).resolve().parent.parent / 'processed_images'), self.num_workers, nargout = 0) 
